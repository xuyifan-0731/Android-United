...
def make_merge_pic(log_path, save_path=None):
    ...
    with open(trace_file, 'r', encoding=trace_file_encoding) as f:
        for obj in f:
            obj = json.loads(obj)
            ...
            if parsed_action["action"] == "Tap":
                if len(parsed_action["kwargs"]["element"]) == 4:
                    ...
                    start_pos = parsed_action["kwargs"]["element"]

                processed_img = draw_cross_on_image(img, start_pos)
            elif parsed_action["action"] == "Swipe":
                text = f"{parsed_action['action']}: {parsed_action['kwargs']}"
                text_img = create_text_image(text, img, 48, log_path=log_path)
                start_pos = (
                    (parsed_action["kwargs"]["element"][0] + parsed_action["kwargs"]["element"][2])/2,
                    (parsed_action["kwargs"]["element"][1] + parsed_action["kwargs"]["element"][3])/2)
                #end_pos = (parsed_action["position_end"][0], parsed_action["position_end"][1] )
                processed_img = draw_cross_on_image(img, start_pos)
                processed_img = merge_text(processed_img, text_img, position=(0, 0))
            ...
            elif parsed_action["action"] in ["press_key"]:
                text = f"{parsed_action['type']}: {parsed_action.get('input', parsed_action.get('key_name', ''))}"
                text_img = create_text_image(text, img, 48, log_path=log_path)
                processed_img = merge_text(img, text_img, position=(0, 0))
            elif parsed_action["action"] in ["Press Home", "Press Back", "Press Enter"]:
                text = f"{parsed_action['action']}: {parsed_action['action']}"
                text_img = create_text_image(text, img, 48, log_path=log_path)
                processed_img = merge_text(img, text_img, position=(0, 0))
            elif parsed_action["action"] == "Launch":
                text = f"{parsed_action['action']}: {parsed_action['kwargs']['package']}"
                text_img = create_text_image(text, img, 48, log_path=log_path)
                processed_img = merge_text(img, text_img, position=(0, 0))
                ...
                text_img = create_text_image(text, img, 48, log_path=log_path)
                processed_img = merge_text(img, text_img, position=(0, 0))
                #print(f"Unknown action: {parsed_action}")
...
def check_all_log(all_log_path, save_path=None):
    ...
    if os.path.exists(save_path):
        exist_result = os.listdir(save_path)
        exist_result = [i.split("_final")[0] for i in exist_result]
        to_process = os.listdir(all_log_path)
        to_process = [i for i in to_process if i not in exist_result]
    else:
        to_process = os.listdir(all_log_path)
    with Pool(processes=16) as pool:
        for log in tqdm(to_process):
...
if __name__ == '__main__':
    # 将directory_path和save_path修改为参数传入
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--directory_path', type=str, default='logs/evaluation/')
    parser.add_argument('--save_path', type=str, default='logs/pic')
